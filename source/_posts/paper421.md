...
设置是否使用卫星天线相位中心变化值。RTK模式下可关闭它，但做PPP的时候需要打开。如果设置为on时则需要在files参数中指定卫星的天线PCV文件。

### pos1-posopt2 = off，on（Rec PCV）

设置是否使用接收器天线相位中心变化值。如果设置为on，则需要在文件参数中指定接收器天线PCV文件，并在天线部分中指定基站和流动站的接收器天线类型。IGS提供的天线文件中只包含测量级天线，因此当你用的天线在这个列表里的时候就凯瑞用到这些天线文件。它主要影响z轴的精度，因此如果您关心高度，这一点很重要。如果两个两个测站的天线相同，您可以将其关闭，因为它们（通过差分的方式）会被消除。

### pos1-posopt5 = off，on（RAIM FDE）

如果任何卫星的残差超过了一个阈值，则排除该卫星。这这里只会排除那些误差比较大的卫星，这需要比较大的运算量，所以我通常会将这个选项禁用。

### POS1-exclsats =

如果你知道卫星是坏的，你可以通过在这里列出来解决它。如果我怀疑卫星是坏的，我只会在极少数情况下使用它进行调试。

### pos1-navsys = 7,15，

（除常用的GPS系统外）我通常也使用GLONASS和SBAS，因为更多的观测信息能带来更好的结果。如果在M8T上使用较新的3.0 u-blox固件，我也会启用Galileo系统。
...
整周模糊度解算方法。`continuous`模式不利用固定解来调整相位偏差状态，所以它对假修复最不敏感。`fix-and-hold`确实利用固定解的反馈来帮助跟踪模糊度。我更喜欢使用`fix-and-hold`并将跟踪增益（pos2-varholdamb）调整得足够低，以最大限度地减少错误固定的可能性。如果`armode`未设置为`fix-and-hold`，则下面提到的和hold相关的任何选项都不适用，包括pos2-gloarmode。

### pos2-varholdamb = 0.001,0.1（meter）

在demo5代码中，可以使用此参数来调整`fix-and-hold`的跟踪增益。它实际上是方差而不是增益，因此较大的值会产生较低的增益。0.001是默认值，任何超过100的效果都很小。该值用作在保持期间产生的伪测量的方差，其提供反馈以将卡尔曼滤波器中的偏置状态朝向整数值驱动。我发现从0.1到1.0的值提供足够的增益来辅助跟踪，同时在大多数情况下仍然避免跟踪错误修复。
